...
class MyEnv(gym.Env):
    ...
    def __init__(
        ...
        transition_probabilities : np.ndarray, optional
            A (2 (num of states), 2 (num of actions)) array specifying the probability of actions being followed.
        rewards : list of float, optional
            Rewards assigned to each action, by default [0,1]
        ...
        self.position = 0  # always start at state 0
    ...
    def step(
        ...
        reward : float
            The reward by taking the action.
        terminated : bool
        ...
        # new position equals action taken
        self.position = a_used

        reward = float(self.rewards[a_used])
        terminated = False
    ...
    def get_reward_per_action(self) -> np.ndarray:
        ...
        R[s, a] is the reward for performing the action a.
